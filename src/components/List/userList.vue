    ...
    <el-card>
      ...
      <!-- 用户列表区域 -->
            <el-table :data="userlist" border stripe>
                ...
                <el-table-column label="注册时间" prop="registe_time" ></el-table-column>
...
export default {
  ...
  methods: {
    ...
    getUserList: async function () {
      const { data: res } = await this.$http.get('/v1/users/list', { params: this.queryInfo })
      console.log(res)
      this.userlist = res
    },
    // 获取用户数量信息
    getCount: async function () {
      const { data: countData } = await this.$http.get('/v1/users/count')
